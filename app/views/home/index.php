      ...
      <?php foreach ($tweets as $tweet): ?>
        <div class="post">
          ...
          <div class="post__body">
            ...
            <form id="editTweetForm-<?php echo $tweet['TweetID']; ?>" class="edit-tweet-form" style="display: none;">
            ...
            <form id="addCommentForm-<?php echo $tweet['TweetID']; ?>" style="margin-top: 10px;"
              onsubmit="return addComment(<?php echo $tweet['TweetID']; ?>)">
              <input type="text" id="commentContent-<?php echo $tweet['TweetID']; ?>" placeholder="Add a comment">
...
<script>
  ...
  function addComment(tweetId) {
    var commentContent = document.getElementById('commentContent-' + tweetId).value;
    var xhr = new XMLHttpRequest();
    xhr.open('POST', 'index.php?controller=TweetController&action=addComment', true);
    xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
    xhr.onreadystatechange = function () {
      if (xhr.readyState === XMLHttpRequest.DONE && xhr.status === 200) {
        // Update the comments section with the newly added comment
        document.getElementById('comments-' + tweetId).innerHTML += '<p>' + commentContent + '</p>';
        // Clear the comment input field
        document.getElementById('commentContent-' + tweetId).value = '';
      }
    };
    xhr.send('tweet_id=' + tweetId + '&content=' + commentContent);
